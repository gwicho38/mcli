    ...
    async def _status():
        ...
        if status['running']:
            ...
            if status.get('keyspace_hits', 0) + status.get('keyspace_misses', 0) > 0:
                hit_rate = status.get('keyspace_hits', 0) / (status.get('keyspace_hits', 0) + status.get('keyspace_misses', 0)) * 100
...
def test_redis():
    ...
    async def _test():
        ...
        if result['status'] == 'success':
            ...
            click.echo(f"   Read/Write Test: {'✅ Passed' if result['test_result'] else '❌ Failed'}")
        else:
    ...
    async def _info():
        ...
        try:
            ...
            if info.get('keyspace_hits', 0) + info.get('keyspace_misses', 0) > 0:
                hit_rate = info.get('keyspace_hits', 0) / (info.get('keyspace_hits', 0) + info.get('keyspace_misses', 0)) * 100
