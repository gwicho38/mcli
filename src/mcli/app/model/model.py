...
class ComfyUIClient:
    ...
    def wait_for_prompt(self, prompt_id: str) -> Dict[str, Any]:
        ...
        with click.progressbar(
            length=100,
            label=f"Processing frame with ComfyUI (ID: {prompt_id})"
        ) as bar:
            progress = 0
            while progress < 100:
    ...
    def build_frame_processing_workflow(self,
                                       filename: str,
                                       subfolder: str,
                                       prompt: str,
                                       negative_prompt: str,
        ...
        workflow = {
            ...
            "5": {
                "inputs": {
                    "model_name": os.path.basename(CONFIG["hunyuan_video_model"])
                },
                "class_type": "HunyuanVideoModelLoader"
            },
            "6": {
                "inputs": {
                    "vae_name": os.path.basename(CONFIG["hunyuan_vae"])
                },
        ...
        if use_controlnet:
            workflow["12"] = {
                "inputs": {
                    "control_net_name": os.path.basename(CONFIG["controlnet_model"])
                },
...
class VideoToVideoGenerator:
    ...
    def generate(self,
        ...
        click.echo(click.style(f"Video-to-Video Generation: {input_video} → {output_video}", fg="bright_blue"))
        click.echo(click.style("=" * 80, fg="bright_blue"))
    ...
    def process_frames(self,
        ...
        if batch_size <= 1:
            ...
            for i in range(0, len(frame_paths), batch_size):
                ...
                click.echo(f"Processing batch {i//batch_size+1}/{(len(frame_paths)-1)//batch_size+1} ({len(batch)} frames)")
                ...
                for j, frame_path in enumerate(batch):
                    thread = threading.Thread(
                        ...
                        args=(frame_path, prompt, negative_prompt, strength, 
                              guidance_scale, use_controlnet, use_lora, 
                              i+j, results_queue)
                    )
                ...
                with click.progressbar(
                    length=len(batch),
                    label=f"Batch {i//batch_size+1} progress"
                ) as bar:
                    completed = 0
...
def generate(input, output, prompt, negative, fps, strength, guidance, batch, 
             no_controlnet, no_lora, no_temporal, model, lora_model, controlnet_model):
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 200:
                ...
                if not hunyuan_found:
                    click.echo(click.style("⚠ Warning: No Hunyuan models found in ComfyUI", fg="yellow"))
                    click.echo("  Make sure you have the Hunyuan video models installed in your ComfyUI setup")
            else:
