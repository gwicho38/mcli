...
def list_commands(include_groups: bool, daemon_only: bool, as_json: bool):
    ...
    try:
        ...
        if as_json:
            click.echo(json.dumps({"commands": commands_data, "total": len(commands_data)}, indent=2))
            return
...
def search_commands(query: str, daemon_only: bool, as_json: bool):
    ...
    try:
        if daemon_only:
            ...
            matching_commands = [
                ...
                if (query.lower() in cmd['name'].lower() or
                    query.lower() in (cmd['description'] or "").lower() or
                    any(query.lower() in tag.lower() for tag in cmd.get('tags', [])))
            ]
        else:
        ...
        if as_json:
            click.echo(json.dumps({"commands": matching_commands, "total": len(matching_commands), "query": query}, indent=2))
            return
