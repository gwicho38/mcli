...
from mcli.workflow.scheduler.job import ScheduledJob, JobStatus, JobType
...
@click.option(
    "--quick", 
    is_flag=True, 
    help="Run quick validation (30 seconds) instead of full test"
)
@click.option(
    "--cleanup", 
    is_flag=True, 
    help="Clean up test jobs and files after validation"
)
@click.option(
    "--verbose", 
    "-v", 
...
def test_job_scheduling(scheduler: JobScheduler, storage: JobStorage, test_jobs: list, verbose: bool) -> bool:
...
def test_manual_execution(scheduler: JobScheduler, test_job_config: dict, test_file: Path, verbose: bool) -> bool:
...
def monitor_job_execution(scheduler: JobScheduler, storage: JobStorage, duration: int, test_file: Path, verbose: bool):
...
def analyze_job_completions(storage: JobStorage, test_file: Path, verbose: bool) -> bool:
    ...
    try:
        ...
        test_jobs = [job for job in all_jobs if hasattr(job, 'name') and ('cron_test' in job.name or 'manual_test' in job.name)]
...
def analyze_individual_job(job: ScheduledJob, verbose: bool):
    ...
    if hasattr(job, 'last_output') and job.last_output and verbose:
        output_preview = job.last_output[:200] + "..." if len(job.last_output) > 200 else job.last_output
    ...
    # Command details
    if verbose:
        console.print(f"      Command: {job.command[:100]}{'...' if len(job.command) > 100 else ''}")
        if job.description:
...
def analyze_execution_output(test_file: Path, verbose: bool):
    ...
    try:
        ...
        console.print(f"    📅 Created: {datetime.fromtimestamp(file_stats.st_ctime).strftime('%H:%M:%S')}")
        console.print(f"    🔄 Modified: {datetime.fromtimestamp(file_stats.st_mtime).strftime('%H:%M:%S')}")
...
def calculate_performance_metrics(test_jobs: list, verbose: bool):
    ...
    runtime_jobs = [job for job in test_jobs if hasattr(job, 'runtime_seconds') and job.runtime_seconds > 0]
    if runtime_jobs:
...
def show_test_summary(storage: JobStorage, test_jobs: list):
    ...
    try:
        all_jobs = storage.load_jobs()
        test_job_count = sum(1 for job in all_jobs if hasattr(job, 'name') and 'cron_test' in job.name)
