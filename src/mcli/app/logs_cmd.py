...
def stream_logs(type: str, lines: int, follow: bool):
    ...
    if not logs_dir.exists():
        console.print("❌ No logs directory found. Run some MCLI commands to generate logs.", style="red")
        return
...
@click.option(
    "--date", "-d",
    help="Show logs for specific date (YYYYMMDD format)"
)
...
@click.option(
    "--date", "-d",
    help="Date for log file (YYYYMMDD format, default: today)"
)
...
@click.option(
    "--date", "-d",
    help="Date for log files (YYYYMMDD format, default: today)"
)
@click.option(
    "--context", "-C",
    type=int,
...
@click.option(
    "--older-than",
    type=int,
    help="Clear logs older than N days"
)
@click.option(
    "--confirm", "-y",
    is_flag=True,
...
def clear_logs(older_than: Optional[int], confirm: bool):
    ...
    console.print(f"✅ Deleted {deleted_count} log files ({_format_file_size(total_size)} freed)", style="green")
...
def _list_available_logs(logs_dir: Path, date_filter: Optional[str] = None):
    ...
    for log_file in log_files:
        ...
        console.print(f"   • {log_file.name} ({_format_file_size(size)}, {mtime.strftime('%Y-%m-%d %H:%M')})")
