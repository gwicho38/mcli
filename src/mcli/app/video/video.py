...
class VideoProcessor:
    ...
    def frames_to_video(self, frame_paths: List[str], output_path: str, fps: float = None) -> str:
        ...
        click.echo(click.style(f"Converting {len(frame_paths)} frames to video at {fps} FPS...", fg="green"))
    ...
    def apply_temporal_consistency(self, processed_frames: List[str], motion_data: Dict[str, Any]) -> List[str]:
        """
        ...
        with click.progressbar(
            ...
            length=len(processed_frames),
            label="Reducing flicker"
        ) as bar:
            for i, frame_path in bar:
                ...
                if prev_frame is not None:
                    ...
                    if f"frame_{i-1:05d}" in motion_data:
                        ...
                        motion_magnitude = np.sqrt(motion_info["mean_x"]**2 + motion_info["mean_y"]**2)
                    ...
                    blended_frame = cv2.addWeighted(
                        prev_frame, blend_weight, 
                        frame, 1.0 - blend_weight, 
                        0
                    )
                    frame = blended_frame
...
class OverlayRemover:
    ...
    def detect_green_overlay(self, frame: np.ndarray, method: str = "hsv") -> np.ndarray:
        ...
        # Remove small noise
        mask = morphology.remove_small_objects(mask.astype(bool), min_size=50).astype(np.uint8) * 255
    ...
    def inpaint_frame(self, frame: np.ndarray, mask: np.ndarray, method: str = "telea") -> np.ndarray:
        """
    ...
    def remove_overlay_from_frame(self, frame: np.ndarray, debug: bool = False) -> Tuple[np.ndarray, np.ndarray]:
        """
...
class EnhancedVideoProcessor(VideoProcessor):
    ...
    def process_frames_remove_overlay(self, frame_paths: List[str], save_debug: bool = False) -> List[str]:
        """
    ...
    def analyze_overlay_consistency(self, frame_paths: List[str]) -> Dict[str, Any]:
        ...
        for i, frame_path in enumerate(frame_paths[:min(50, len(frame_paths))]):  # Sample first 50 frames
            frame = np.array(Image.open(frame_path))
    ...
    def remove_overlay_from_video(self, video_path: str, output_path: str = None, fps: int = 8, apply_temporal_smoothing: bool = True) -> str:
        """
        ...
        try:
            ...
            motion_data = self.extract_motion_vectors(video_path) if apply_temporal_smoothing else None
            ...
            if apply_temporal_smoothing and motion_data:
                final_frame_paths = self.apply_temporal_consistency(cleaned_frame_paths, motion_data)
            else:
            ...
            click.echo(click.style(f"✅ Overlay removal complete! Output: {output_video}", fg="bright_green"))
            return output_video
...
class AdvancedOverlayRemover:
    ...
    def detect_green_overlay(self, frame: np.ndarray, method: str = "combined") -> np.ndarray:
        ...
        mask = morphology.remove_small_objects(mask.astype(bool), min_size=30).astype(np.uint8) * 255
        mask = morphology.remove_small_holes(mask.astype(bool), area_threshold=100).astype(np.uint8) * 255
        
        return mask
    
    def find_similar_patches(self, frame: np.ndarray, mask: np.ndarray, target_point: Tuple[int, int]) -> np.ndarray:
        ...
        for sy in range(search_y_start, search_y_end, 2):  # Step by 2 for speed
            for sx in range(search_x_start, search_x_end, 2):
                ...
                if len(target_known) > 0:
                    correlation = np.corrcoef(target_known.flatten(), candidate_known.flatten())[0, 1]
                    ...
                    if mask_to_fill[py, px] > 0:  # Need to fill this pixel
                        ...
                        if similar_patches:
                            ...
                            if np.any(mask_indices):
                                result[py:y_end, px:x_end][mask_indices] = blended_patch[:y_end-py, :x_end-px][mask_indices]
        
        return result
    
    def temporal_reference_inpainting(self, current_frame: np.ndarray, reference_frames: List[np.ndarray], 
                                    mask: np.ndarray, motion_vectors: Optional[Dict] = None) -> np.ndarray:
        ...
        if np.any(fill_mask):
            ...
            result[fill_mask] = (valid_pixels[fill_mask] / weight_sum_expanded[fill_mask]).astype(np.uint8)
    ...
    def content_aware_fill(self, frame: np.ndarray, mask: np.ndarray) -> np.ndarray:
        ...
        result_bgr = (inpainted_ns.astype(np.float32) * texture_mask[:, :, np.newaxis] + 
    ...
    def remove_overlay_with_context(self, frame: np.ndarray, reference_frames: List[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        if reference_frames and len(reference_frames) <= 5:
            cleaned_frame = self.temporal_reference_inpainting(frame, reference_frames, overlay_mask)
        else:
...
class IntelligentVideoProcessor(VideoProcessor):
    ...
    def extract_motion_vectors_highspeed(self, video_path: str, sample_rate: int = 10) -> Dict[str, Any]:
        """
        ...
        with click.progressbar(
            ...
            label="Analyzing motion (sampled)"
        ) as bar:
            while True:
                ret, frame = video.read()
                if not ret:
    ...
    def process_with_temporal_context(self, frame_paths: List[str], context_window: int = 3) -> List[str]:
        """
        ...
        with click.progressbar(
            enumerate(frame_paths),
            length=len(frame_paths),
            label="Intelligent overlay removal"
        ) as bar:
            for i, frame_path in bar:
                # Load current frame
    ...
    def remove_overlay_from_video_intelligent(self, video_path: str, output_path: str = None, 
                                            fps: int = 30, context_window: int = 3) -> str:
        """
        ...
        try:
            ...
            click.echo(click.style(f"📊 Extraction FPS: {fps} (higher = better quality, slower processing)", fg="blue"))
            ...
            motion_data = self.extract_motion_vectors_highspeed(video_path, sample_rate=max(1, fps//6))
            ...
            click.echo(click.style(f"✅ Intelligent overlay removal complete! Output: {output_video}", fg="bright_green"))
            click.echo(click.style(f"⏱️  Original speed maintained using {self.video_info.get('extraction_fps', fps)} FPS", fg="green"))
...
@click.option('--fps', '-f', default=30, help='Frame extraction rate (default: 30 to maintain quality). Higher values = better quality but slower processing')
@click.option('--output-fps', type=float, help='Output video FPS (defaults to extraction FPS for correct speed)')
@click.option('--context', '-c', default=3, help='Temporal context window size (default: 3)')
@click.option('--method', type=click.Choice(['intelligent', 'basic']), default='intelligent',
...
def remove_overlay(input_video, output, fps, output_fps, context, method):
    ...
    if method == 'intelligent':
        ...
        if output_fps and output_fps != fps:
            click.echo(click.style(f"Recreating video with custom output FPS: {output_fps}", fg="yellow"))
            
            # Get the cleaned frames
            cleaned_frames = [os.path.join(processor.cleaned_dir, f) for f in 
                            sorted(os.listdir(processor.cleaned_dir)) if f.endswith('.png')]
            
            # Create new output path
            base_name = os.path.splitext(os.path.basename(input_video))[0]
            custom_output = os.path.join(CONFIG["output_dir"], f"{base_name}_custom_fps_{output_fps}.mp4")
    ...
    click.echo(click.style("📝 Note: Output video should maintain original playback speed", fg="green"))
