...
def remove_overlay(input_video, output, fps, context, method, dry_run):
    ...
    if method == 'intelligent':
        if IntelligentVideoProcessor is None:
            click.echo(click.style("❌ Intelligent video processor not available. Using basic processor.", fg="yellow"))
            processor = EnhancedVideoProcessor() if EnhancedVideoProcessor else VideoProcessor()
        else:
            processor = IntelligentVideoProcessor()
        
        if dry_run:
            click.echo(click.style("🔍 Dry run mode - extracting frames and analyzing video only", fg="cyan"))
            frame_paths = processor.extract_frames(input_video, fps)
            click.echo(click.style(f"✅ Dry run complete. Extracted {len(frame_paths)} frames to {processor.temp_dir}", fg="green"))
        ...
        if hasattr(processor, 'remove_overlay_from_video'):
            ...
            click.echo(click.style("❌ Overlay removal not available with current processor.", fg="red"))
            return
