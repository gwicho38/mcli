...
def test_lightweight_models():
    ...
    try:
        ...
        print(f"🎯 Smallest model: {min(LIGHTWEIGHT_MODELS.items(), key=lambda x: x[1]['size_mb'])[1]['name']}")
        print(f"⚡ Most efficient: {max(LIGHTWEIGHT_MODELS.items(), key=lambda x: x[1]['efficiency_score'])[1]['name']}")
...
def test_model_download():
    ...
    try:
        from mcli.workflow.model_service.lightweight_model_server import LightweightModelDownloader, LIGHTWEIGHT_MODELS
