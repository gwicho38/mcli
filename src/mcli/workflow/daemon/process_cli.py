...
def list_processes(all: bool, as_json: bool):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Print header
            click.echo(f"{'CONTAINER ID':<13} {'NAME':<15} {'COMMAND':<25} {'STATUS':<10} {'UPTIME':<10} {'CPU':<8} {'MEMORY'}")
            click.echo("-" * 90)
            
            # Print process rows
            for proc in processes:
                click.echo(f"{proc['id']:<13} {proc['name']:<15} {proc['command'][:24]:<25} {proc['status']:<10} {proc['uptime']:<10} {proc['cpu']:<8} {proc['memory']}")
        else:
...
def run_process(command: str, args: tuple, name: Optional[str], detach: bool, working_dir: Optional[str]):
