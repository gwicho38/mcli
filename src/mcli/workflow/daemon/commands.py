...
class CommandDatabase:
    ...
    def add_command(self, command: Command) -> str:
        ...
        try:
            ...
            ''', (
                ...
                command.execution_count, command.last_executed.isoformat() if command.last_executed else None,
                command.is_active
            ))
    ...
    def update_command(self, command: Command) -> bool:
        ...
        try:
            ...
            ''', (
                ...
                command.group, json.dumps(command.tags), datetime.now().isoformat(),
                command.is_active, command.id
            ))
    ...
    def record_execution(self, command_id: str, status: str, output: str = None, 
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (execution_id, command_id, datetime.now().isoformat(), 
...
@click.argument('name')
@click.option('--description', help='Command description')
@click.option('--language', type=click.Choice(['python', 'node', 'lua', 'shell']), 
...
def add_interactive():
    ...
    source = click.prompt(
        "Code source",
        type=click.Choice(['file', 'stdin', 'paste']),
        default='paste'
    )
