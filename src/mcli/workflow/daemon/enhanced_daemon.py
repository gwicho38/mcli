...
from mcli.lib.performance.rust_bridge import (
    get_tfidf_vectorizer, get_file_watcher, get_command_matcher, get_process_manager
)
from mcli.lib.search.cached_vectorizer import CachedTfIdfVectorizer, SmartVectorizerManager
from mcli.workflow.daemon.async_command_database import AsyncCommandDatabase, Command, ExecutionRecord
...
class EnhancedDaemon:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.process_manager = AsyncProcessManager(
                db_path=str(self.db_path.parent / "processes.db"),
                redis_url=self.redis_url
            )
    ...
    async def _metrics_loop(self):
        ...
        while self.running:
            try:
                ...
                logger.info(f"Daemon metrics - Uptime: {uptime}, "
    ...
    async def search_commands(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        if commands and self.vectorizer_manager:
            try:
                ...
                results = await self.vectorizer_manager.search_commands(
                    query, command_dicts, limit
                )
    ...
    async def execute_command(self, command_id: str, context: Optional[Dict[str, Any]] = None) -> str:
        """Execute a command"""
