...
class AsyncCommandDatabase:
    ...
    def __init__(self, db_path: Optional[str] = None, redis_url: Optional[str] = None, pool_size: int = 10):
        if db_path is None:
    ...
    async def _init_database(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute('CREATE INDEX IF NOT EXISTS idx_commands_language ON commands(language)')
            await db.execute('CREATE INDEX IF NOT EXISTS idx_commands_group ON commands(group_name)')
            await db.execute('CREATE INDEX IF NOT EXISTS idx_commands_active ON commands(is_active)')
            await db.execute('CREATE INDEX IF NOT EXISTS idx_commands_execution_count ON commands(execution_count)')
            await db.execute('CREATE INDEX IF NOT EXISTS idx_executions_command_id ON executions(command_id)')
            await db.execute('CREATE INDEX IF NOT EXISTS idx_executions_status ON executions(status)')
            await db.execute('CREATE INDEX IF NOT EXISTS idx_executions_executed_at ON executions(executed_at)')
        ...
        async with self._get_connection() as db:
            try:
                await db.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    command.id, command.name, command.description, command.code,
                    command.language, command.group, json.dumps(command.tags),
                    ...
                    command.is_active, command.version, command.author,
        ...
        async with self._get_connection() as db:
            try:
                result = await db.execute('''
                    ...
                    WHERE id=?
                ''', (
                    command.name, command.description, command.code, command.language,
                    command.group, json.dumps(command.tags), command.updated_at.isoformat(),
                    command.execution_count,
                    command.last_executed.isoformat() if command.last_executed else None,
                    command.is_active, command.version, command.author,
                    json.dumps(command.dependencies), command.id
                ))
        ...
        async with self._get_connection() as db:
            try:
                await db.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    execution.id, execution.command_id, execution.executed_at.isoformat(),
                    execution.status, execution.output, execution.error, execution.execution_time_ms,
                    execution.user, json.dumps(execution.context) if execution.context else None
                ))
    ...
    async def get_execution_history(self, command_id: Optional[str] = None, limit: int = 100) -> List[ExecutionRecord]:
    ...
    async def _cache_command(self, command: Command):
        ...
        try:
            command_data = {
                ...
                'last_executed': command.last_executed.isoformat() if command.last_executed else None,
            }
