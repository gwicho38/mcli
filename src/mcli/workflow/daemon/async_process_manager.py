...
class AsyncProcessContainer:
    ...
    async def wait(self, timeout: Optional[float] = None) -> int:
        ...
        self.info.exit_code = self.process.returncode
        self.info.status = ProcessStatus.EXITED if self.process.returncode == 0 else ProcessStatus.FAILED
        self.info.finished_at = datetime.now()
...
class AsyncProcessManager:
    ...
    async def _init_database(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute('CREATE INDEX IF NOT EXISTS idx_processes_created_at ON processes(created_at)')
