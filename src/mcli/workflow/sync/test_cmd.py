...
@test.command()
@click.option('--vault-path', default='.', help='Path to vault directory')
@click.option('--service', type=click.Choice(['github', 'onedrive', 'icloud', 'googledrive', 'all']), 
...
def sync(vault_path, service):
    ...
    try:
        ...
        click.echo("❌ Could not import sync functionality. Ensure vault_sync_standalone.py is available.")
        sys.exit(1)
    ...
    services_to_test = [service] if service != 'all' else ['github', 'onedrive', 'icloud', 'googledrive']
