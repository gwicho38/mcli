...
class MultiCloudSync:
    ...
    def sync_to_github(self) -> bool:
        ...
        try:
            ...
            self.log_sync_action("git_sync", "github", "success", f"Committed and pushed: {commit_msg}")
            return True
    ...
    def sync_to_cloud_storage(self, cloud_name: str) -> bool:
        ...
        if not cloud_path.exists():
            try:
                ...
                self.log_sync_action("cloud_sync", cloud_name, "info", f"Created directory: {cloud_path}")
            except Exception as e:
                self.log_sync_action("cloud_sync", cloud_name, "error", f"Failed to create directory: {e}")
                return False
    ...
    def sync_from_cloud_storage(self, cloud_name: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                self.log_sync_action("cloud_pull", cloud_name, "success", f"Pulled from {cloud_path}")
                return True
            else:
    ...
    for target, info in status_info['targets'].items():
        if info['configured']:
            if target == 'github':
                ...
                click.echo(f"{icon} {target.title()}: {'accessible' if info['accessible'] else 'not accessible'}")
...
@sync.command()
@click.option('--vault-path', default='.', help='Path to vault directory')
@click.option('--target', type=click.Choice(['all', 'github', 'onedrive', 'icloud', 'googledrive']), 
              default='all', help='Sync target')
...
@sync.command()
@click.option('--vault-path', default='.', help='Path to vault directory')
@click.option('--target', type=click.Choice(['onedrive', 'icloud', 'googledrive']), 
              required=True, help='Cloud storage to pull from')
