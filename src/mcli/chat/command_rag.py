...
class CommandRAGSystem:
    ...
    def _generate_examples(self, cmd: DiscoveredCommand, parameters: List[Dict[str, Any]]) -> List[str]:
        ...
        for param in parameters[:3]:  # Limit to first 3 params
            if param.get('is_flag'):
                examples.append(f"{base_cmd} {param['opts'][0] if param['opts'] else '--' + param['name']}")
    ...
    def _find_related_commands(self, cmd: DiscoveredCommand) -> List[str]:
        ...
        for other_cmd in self.discovery.discovered_commands.values():
            if (other_cmd.full_name != cmd.full_name and 
                other_cmd.module_name == cmd.module_name):
    ...
    def _extract_usage_patterns(self, help_text: str, cmd: DiscoveredCommand) -> List[str]:
        ...
        if usage_section:
            usage_lines = [line.strip() for line in usage_section.group(1).split('\n') if line.strip()]
    ...
    async def search_commands(
        ...
        try:
            ...
            documents = [self._create_command_document(ctx) for ctx in self.command_contexts.values()]
    ...
    async def get_command_suggestions(
        ...
        for context, score in search_results:
            suggestion = {
                ...
                'parameters': [p for p in context.parameters if p.get('required')],  # Required params only
                'tags': context.tags
            }
    ...
    async def analyze_user_intent(self, user_message: str) -> Dict[str, Any]:
        ...
        intent_patterns = {
            'start_service': [
                'start', 'run', 'launch', 'begin', 'activate', 'turn on'
            ],
            'stop_service': [
                'stop', 'halt', 'terminate', 'kill', 'shutdown', 'turn off'
            ],
            'status_check': [
                'status', 'info', 'show', 'list', 'check', 'what is', 'tell me about'
            ],
            'help_request': [
                'help', 'how to', 'how do i', 'explain', 'tutorial', 'guide'
            ],
            'performance': [
                'performance', 'optimize', 'speed up', 'faster', 'slow', 'cache'
            ],
            'automation': [
    ...
    def get_system_capabilities(self) -> Dict[str, Any]:
        ...
        for context in self.command_contexts.values():
            ...
            categories[category]['commands'].append({
                'name': context.command.full_name,
                'description': context.command.description
            })
        ...
        return {
            ...
            'available_services': [
                name for name, ctx in self.command_contexts.items() 
