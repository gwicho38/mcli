...
class ChatSystemIntegration:
    ...
    def handle_system_request(self, request: str) -> Dict[str, Any]:
        ...
        if any(phrase in request_lower for phrase in ["what time", "current time", "system time", "what is the time"]):
            return self._handle_system_time_request(request)
        
        elif any(phrase in request_lower for phrase in ["system info", "system information", "system specs", "hardware info"]):
            return self._handle_system_info_request(request)
        
        elif any(phrase in request_lower for phrase in ["memory usage", "ram usage", "how much memory", "how much ram", "memory info"]):
            return self._handle_memory_request(request)
        
        elif any(phrase in request_lower for phrase in ["disk usage", "disk space", "storage space", "how much space", "free space"]):
            return self._handle_disk_request(request)
        
        elif any(phrase in request_lower for phrase in ["clear cache", "clean cache", "clear temp", "free up space", "clean system", "clear system cache"]):
            return self._handle_cache_clear_request(request)
        
        # Navigation requests
        elif any(phrase in request_lower for phrase in ["navigate to", "go to", "change to", "cd to", "move to"]):
            return self._handle_navigation_request(request)
        
        # Directory listing requests  
        elif any(phrase in request_lower for phrase in ["list", "show files", "ls", "dir", "what's in"]):
            return self._handle_directory_listing_request(request)
        
        # Simulator cleanup requests
        elif any(phrase in request_lower for phrase in ["clean simulator", "simulator data", "clean ios", "clean watchos"]):
            return self._handle_simulator_cleanup_request(request)
        
        # Shell command requests
        elif any(phrase in request_lower for phrase in ["run command", "execute", "shell", "terminal"]):
            return self._handle_shell_command_request(request)
        
        # Current directory requests
        elif any(phrase in request_lower for phrase in ["where am i", "current directory", "pwd", "current path"]):
        ...
        # File/URL opening
        elif "open" in request_lower and ("file" in request_lower or "url" in request_lower or "http" in request_lower):
    ...
    def _handle_open_request(self, request: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Error opening file/URL: {e}",
                "request": request
            }
    ...
    def _handle_system_time_request(self, request: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                ...
                result["message"] = f"⏰ Current time: {time_data['current_time']} ({time_data['timezone'][0]})"
        ...
        try:
            ...
            if result["success"]:
                ...
                if disk_data["total_disk_gb"] > 0:
                    ...
                    summary.append(f"  Main: {disk_data['total_used_gb']:.1f}GB used / {disk_data['total_disk_gb']:.1f}GB total ({usage_pct:.1f}%)")
                ...
                if len(disk_data["partitions"]) > 1:
                    ...
                    for partition in disk_data["partitions"]:
                        if partition["mountpoint"] not in ["/", "C:\\"]:
                            summary.append(f"    {partition['mountpoint']}: {partition['used_gb']:.1f}GB / {partition['total_gb']:.1f}GB ({partition['usage_percent']}%)")
            ...
            return {
                "success": False,
                "error": f"Error getting disk usage: {e}",
                "request": request
            }
    ...
    def _handle_cache_clear_request(self, request: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Error clearing caches: {e}",
                "request": request
            }
    ...
    def _handle_directory_listing_request(self, request: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Directory listing error: {e}",
                "request": request
            }
    ...
    def _handle_simulator_cleanup_request(self, request: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                data = result["data"]
                result["message"] = f"🧹 Simulator cleanup completed!\n💾 Freed {data['total_freed_mb']} MB of storage"
            ...
            return {
                "success": False,
                "error": f"Simulator cleanup error: {e}",
                "request": request
            }
    ...
    def _handle_current_directory_request(self, request: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Current directory error: {e}",
                "request": request
            }
