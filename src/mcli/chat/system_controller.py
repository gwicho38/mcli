...
class SystemController:
    ...
    def open_textedit_and_write(self, text: str, filename: str = None) -> Dict[str, Any]:
        ...
        return self.execute_command(
            ...
            f"Open TextEdit and write: {text[:50]}..."
        )
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cpu_info = {
                ...
                "cpu_usage_percent": psutil.cpu_percent(interval=0.1),  # Shorter interval to avoid hanging
                "cpu_frequency": None,
            }
    ...
    def get_memory_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if memory.percent > 80:
                memory_info["recommendations"].append("High memory usage detected - consider closing unused applications")
            
            if memory.percent > 90:
                memory_info["recommendations"].append("Critical memory usage - immediate cleanup recommended")
                ...
                try:
                    ...
                    if partition.mountpoint == "/" or partition.mountpoint == "C:\\":
                        ...
                        if partition_info["usage_percent"] > 85:
                            disk_info["recommendations"].append(f"Disk {partition.mountpoint} is {partition_info['usage_percent']}% full - cleanup recommended")
                        
                        if partition_info["usage_percent"] > 95:
                            disk_info["recommendations"].append(f"Critical: Disk {partition.mountpoint} is nearly full!")
    ...
    def clear_system_caches(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.system == "Darwin":  # macOS
                ...
                for cache_dir in cache_dirs:
                    ...
                    if os.path.exists(expanded_dir):
                        try:
                            ...
                            if cache_dir == "/tmp":
                                self.execute_command("find /tmp -type f -atime +1 -delete 2>/dev/null || true", "Clear old temp files")
                            else:
                                # Just clear user cache files that are safe to delete
                                self.execute_command(f"find '{expanded_dir}' -name '*.cache' -delete 2>/dev/null || true", f"Clear {cache_dir}")
                ...
                # Clear DNS cache
                dns_result = self.execute_command("sudo dscacheutil -flushcache 2>/dev/null || dscacheutil -flushcache", "Flush DNS cache")
                ...
                commands = [
                    ("find /tmp -type f -atime +1 -delete 2>/dev/null || true", "Clear old temp files"),
                    ("sync && echo 3 > /proc/sys/vm/drop_caches 2>/dev/null || true", "Clear page cache"),
                ]
    ...
    def open_file_or_url(self, path_or_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": str(e),
                "description": f"Open: {path_or_url}"
            }
    ...
    def list_directory(self, path: str = None, show_hidden: bool = False, detailed: bool = False) -> Dict[str, Any]:
...
def list_directory(path: str = None, show_hidden: bool = False, detailed: bool = False) -> Dict[str, Any]:
