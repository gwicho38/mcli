...
from typing import Callable, Dict, Optional, Any, Set, List, Union, Tuple
...
class McliLogger:
    ...
    def enable_runtime_tracing(cls, level: int = 1, excluded_modules: Optional[List[str]] = None):
        ...
        if excluded_modules:
            ...
            cls._excluded_modules = {
                'logging', 'importlib', 'typing', 'abc', 'inspect', 'pkg_resources',
                'encodings', '_weakrefset', 'weakref', 'sre_', 're', 'functools',
                'threading', 'copyreg', 'collections', '_collections_abc', 'enum'
            }
        ...
        if cls._trace_level > 0 and not cls._runtime_tracing_enabled:
            ...
            cls._instance.trace_logger.info(f"Python interpreter tracing enabled (level={cls._trace_level})")
        ...
        if cls._system_trace_level > 0 and not cls._system_tracing_enabled:
            if cls._system_trace_thread is None or not cls._system_trace_thread.is_alive():
                ...
                cls._system_trace_thread = threading.Thread(
                    target=cls._instance._system_trace_worker,
                    daemon=True
                )
    ...
    def __init__(self, name="mcli.out"):
        ...
        try:
            ...
            fallback_formatter = logging.Formatter('[FALLBACK] %(asctime)s [%(levelname)s] %(message)s')
            ...
            if detailed:
                try:
                    ...
                    detailed_info = {
                        ...
                        "children": [{"pid": child.pid, "name": child.name()} for child in children[:5]]  # First 5 children
                    }
    ...
    def _system_trace_worker(self):
        ...
        while self._system_tracing_enabled:
            try:
                ...
                current_pids = set(self._system_trace_process_ids)  # Make a copy to avoid modification during iteration
                ...
                for pid in current_pids:
                    try:
                        ...
                        if process_info.get("status") != "NOT_FOUND":
                            ...
                            self.system_trace_logger.info(f"Process {pid} no longer exists, removing from monitoring")
                            ...
                            try:
                                ...
                                for child in children:
                                    ...
                                    if child.pid not in self._system_trace_process_ids:
                                        ...
                                        self.system_trace_logger.info(f"Added child process {child.pid} ({child.name()}) to monitoring")
                            except Exception as e:
                                self.system_trace_logger.error(f"Error getting child processes for {pid}: {e}")
                                
                    except Exception as e:
                        self.system_trace_logger.error(f"Error in system trace for process {pid}: {e}")
        ...
        try:
            ...
            if event == 'call' and self._trace_level >= 1:
                ...
                if self._trace_level >= 3:
                    ...
                    self.trace_logger.debug(f"CALL {module}.{function}({args_repr}) at {filename}:{lineno}")
                else:
                ...
                if self._trace_level >= 3:
                    ...
                    try:
                        with open(filename, 'r') as f:
                            lines = f.readlines()
                            source = lines[lineno-1].strip() if lineno <= len(lines) else "<source not available>"
