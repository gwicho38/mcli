...
class CachedTfIdfVectorizer:
    ...
    async def similarity_search(self, query: str, documents: List[str], top_k: int = 10) -> List[Tuple[int, float]]:
    ...
    async def get_cache_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'vectorizer_type': 'rust' if self.use_rust and 'mcli_rust' in str(type(self.vectorizer)) else 'sklearn'
        }
    ...
    async def warm_cache(self, documents: List[str], common_queries: List[str]):
        ...
        logger.info(f"Warming cache with {len(common_queries)} queries and {len(documents)} documents")
...
class SmartVectorizerManager:
    ...
    async def get_vectorizer(self, 
                           ...
                           ngram_range: Tuple[int, int] = (1, 2)) -> CachedTfIdfVectorizer:
    ...
    async def search_commands(self, 
                            query: str, 
