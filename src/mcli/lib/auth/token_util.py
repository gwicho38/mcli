...
def _generate_signature(private_key_path):
    ...
    sig = os.popen('logger.infof ' + nonce + ' | openssl dgst -hex -sigopt rsa_padding_mode:pss -sha256 -sign ' + private_key_path).read()
...
class Cloud:
    ...
    def __init__(self, name, file_system_name, credentials_file_name, provisioned_credentials_subpath):
        # name of the Cloud
        self.name = name
    ...
    def writeFileSystemConfig(self):
        logger.info(f'Setting {self.file_system_name} as default filesystem at the _cluster_ override')
        ...
        writeJsonToFile(file_system_config_dir, file_system_config_path, file_system_config_json_map)

    def writeCredentials(self):
        # write provisioned credentials to the config framework
        provisioned_credentials_path = os.path.join(DEV_SECRETS_ROOT, self.provisioned_credentials_subpath)
        ...
        with open(provisioned_credentials_path, 'r') as provisioned_credentials_file:
            ...
            if self.name == CloudName.AWS.value:
                ...
                storageCredentials = {}
                storageCredentials['accountName'] = provisioned_credentials_mapping['storage_account_name']
                storageCredentials['accessKey'] = provisioned_credentials_mapping['storage_access_key']
...
def configure_k8s_context(namespace, context):
    ...
    logger.debug('Configuring mcli Server to use k8s namespace "%s" in context %s', namespace, context)
...
def load_mcli(host):
    ...
    try:
        ...
        logger.error(f'Cannot connect to mcli server on {url}\nPlease, ensure mcli server is running and try again.')
        sys.exit(1)
    except BaseException:
...
def get_current_version(file_path = VERSION_FILE_PATH):
    ...
    if version == None:
        ...
        raise ValueError(f"File '{file_path}' does not have a valid <major>.<minor>.<patch> version. Version: '{version}'")
...
def align_version(files, platform_pkgs, version, compatible_to_version, ui_pkgs=set(), ui_compatible_to_version=None, is_platform=False):
    """
        ...
        with open(file_path, "r", encoding="utf-8") as file:
            try:
                ...
                if is_platform:
                    ...
                    if K_VERSION not in data or data[K_VERSION] == None or data[K_VERSION] != version:
                    ...
                    if K_COMPAT not in data or data[K_COMPAT] == None or data[K_COMPAT] != compat_version:
...
def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(description="Script to align the platform pkgDecl versions with version.txt")

        parser.add_argument("--compatibleToVersion", type=str, help="The compatiableVersionTo for ALL platform pkgs. Default is \"platform version\" - 1.")
        parser.add_argument("--uiCompatibleToVersion", type=str, help="The compatiableVersionTo for ALL UI pkgs. Default is the same as --compatibleToVersion. (optional)")
        ...
        compatible_to_version = args.compatibleToVersion if args.compatibleToVersion is not None else previous_version(version.rsplit(".", 1)[0])
        ui_compatible_to_version = args.uiCompatibleToVersion if args.uiCompatibleToVersion is not None else compatible_to_version

        # Ensure Version in platform pkgs (align version, compatibleVersionTo and dependencies)
        align_version(pkg_jsn_files, pkgs, version, compatible_to_version, ui_pkgs, ui_compatible_to_version, True)
