...
from typing import Dict, List, Set, Tuple, Union, Optional, Any, Protocol
...
def do_erd(max_depth=1, type_system: Optional[TypeSystem] = None):
    ...
    if mode == "realGraph":
        ...
        try:
            ...
            if result:
                ...
                logger.warning("Failed to generate ERD with realGraph.json approach, falling back to MCLI connection method.")
                mode = "mcli"
        except Exception as e:
    ...
    if mode == "mcli":
        ...
        if mode == "type":
            ...
            try:
                ...
                for attr_name in dir(mcli):
                    if not attr_name.startswith('_') and attr_name not in ERD.system_types:  # Skip private attributes and system types
                        try:
            ...
            try:
                ...
                if len(pkg_types) > 10:
                    include_all = click.confirm(f"Package contains {len(pkg_types)} types. Include all in ERD?", default=False)
                    if not include_all:
            ...
            try:
                ...
                include_pkg_types = click.confirm("Would you like to include types from packages as well?", default=True)
                if include_pkg_types:
                    ...
                    if packages:
                        ...
                        selected_pkg = click.prompt("Enter package name to include (or 'all' for all packages)", default="all")
                        
                        if selected_pkg.lower() == "all":
                            # Add types from all packages
                            for pkg_name in packages:
                                pkg_type_set = ERD.get_pkg_types(type_system, pkg_name)
                                click.echo(f"Adding {len(pkg_type_set)} types from package {pkg_name}")
                            ...
                            if selected_pkg in packages:
                                ...
                                click.echo(f"Adding {len(pkg_type_set)} types from package {selected_pkg}")
                                pkg_types.update(pkg_type_set)
                            else:
                                click.echo(f"Package '{selected_pkg}' not found, using only root namespace types")
            except AttributeError:
                click.echo(f"Error: Type '{type_name}' not found")
                return
        ...
        if pkg_types and mode != "type":
            click.echo(f"Including {len(pkg_types)} types from {'package' if mode == 'package' else 'root namespace'}")
        ...
        # Create a new graph
        graph = pydot.Dot(
            graph_type="digraph", rankdir="TB", splines="ortho", bgcolor="white"
        )
...
def create_merged_erd(types: List[str], type_system: Optional[TypeSystem] = None, mcli=None, env_url: str = None, max_depth: int = 2, 
                      output_prefix: str = "MergedERD", include_methods: bool = False) -> Tuple[str, str]:
    """
    ...
    to_process = [(name, obj, 0) for name, obj in root_types]  # (type_name, type_obj, current_depth)
    ...
    process_types_to_depth(to_process, processed_types, type_depth, entities, type_system, max_depth)
...
def build_relationship_map(entities: Dict[str, Dict]) -> Dict[str, Set[str]]:
    ...
    for entity_name, entity_data in entities.items():
        ...
        for field_name, field_type in fields:
            ...
            if target_type in ["StringType", "IntegerType", "DoubleType", "DateTimeType", "BooleanType"]:
                continue
...
def generate_merged_erd_for_types(type_names: List[str], env_url: str = None, type_system: Optional[TypeSystem] = None, max_depth: int = 2, 
                               output_prefix: str = "MergedERD") -> Tuple[str, str]:
    """
...
def generate_erd_for_top_nodes(graph_file_path: str, max_depth: int = 2, top_n: int = 50) -> List[Tuple[str, str, int]]:
    """
    ...
    try:
        ...
        logger.info(f"Found {len(top_nodes)} top nodes in the graph: {', '.join([node_id for node_id, _ in top_nodes])}")
        
        # Generate ERDs for each top node
        from mcli.app.readiness.generate_graph import (
            build_adjacency_list, build_hierarchical_graph, create_dot_graph
        )
        ...
        logger.info(f"Building hierarchical graph with {len(top_node_ids)} root nodes and max depth {max_depth}")
        ...
        for root_node_id, descendant_count in top_nodes:
            try:
                ...
                logger.info(f"Generated graph for {root_node_id} with {descendant_count} descendants: {png_filename}")
            except Exception as e:
        ...
        for node_id, descendant_count in top_nodes:
            try:
                ...
                results.append({
                    ...
                    "direct_children": direct_children[:10]  # Limit to first 10 children for brevity
                })
                
                logger.info(f"Top node: {node_id} - {name} - {descendant_count} descendants, {len(direct_children)} direct children")
            except Exception as e:
