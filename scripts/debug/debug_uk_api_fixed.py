            ...
            if categories:
                ...
                async with session.get(interests_url, params=params) as response:
                    ...
                    if response.status == 200:
                        ...
                        print(f"Found {len(interests)} interests in category '{first_cat.get('name')}':")
                        for i, interest in enumerate(interests):
                            ...
                            print(f"    Date: {interest.get('registeredLate') or interest.get('dateCreated')}")
                            ...
                            if interests:
                                ...
                                if member_id:
                                    ...
                                    async with session.get(member_url) as member_response:
                                        print(f"Member API response status: {member_response.status}")
                                        if member_response.status == 200:
                                            ...
                                            for key in ["name", "displayAs", "nameGiven", "nameFull", "nameFamily"]:
                                                if key in member_data:
                                                    print(f"    {key}: {member_data[key]}")
                                        else:
                                            response_text = await member_response.text()
                                            print(f"Failed to fetch member {member_id}: HTTP {member_response.status}")
                                            print(f"Response: {response_text[:200]}")
