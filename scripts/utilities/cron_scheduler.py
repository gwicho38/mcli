...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/tmp/politician_trading_cron.log"), logging.StreamHandler()],
)
...
class PoliticianTradingCronManager:
    ...
    async def run_scheduled_collection(self, collection_type: str = "full") -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Collection completed - New: {summary.get('total_new_disclosures', 0)}, "
...
class DatabaseReader:
    ...
    async def get_recent_disclosures(
        self, limit: int = 50, days_back: int = 30
    ) -> list[dict[str, Any]]:
    ...
    def _is_recent(self, timestamp_str: str, hours_threshold: int = 24) -> bool:
        ...
        try:
            ...
            return (datetime.now() - timestamp.replace(tzinfo=None)) < timedelta(
...
async def read_recent_data():
    ...
    for job in jobs[:5]:
        ...
        print(
            f"{status_icon} {job.get('job_type', 'unknown').replace('_', ' ').title()}: {job.get('started_at', 'N/A')[:19]}"
        )
    ...
    for source, info in freshness.items():
        ...
        print(
            f"{status_icon} {source.replace('_', ' ').title()}: {info['last_success'][:19] if info['last_success'] else 'Never'}"
        )
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if command == "full":
            ...
            print(
                "0 */6 * * * cd /Users/lefv/repos/mcli && source .venv/bin/activate && python cron_scheduler.py full >> /tmp/politician_cron.log 2>&1"
            )
            print()
            print("# US collection every 4 hours")
            print(
                "0 */4 * * * cd /Users/lefv/repos/mcli && source .venv/bin/activate && python cron_scheduler.py us >> /tmp/politician_cron.log 2>&1"
            )
            print()
            print("# EU collection every 8 hours")
            print(
                "0 */8 * * * cd /Users/lefv/repos/mcli && source .venv/bin/activate && python cron_scheduler.py eu >> /tmp/politician_cron.log 2>&1"
            )
            print()
            print("# Read data summary daily")
            print(
                "0 9 * * * cd /Users/lefv/repos/mcli && source .venv/bin/activate && python cron_scheduler.py read >> /tmp/politician_cron.log 2>&1"
            )
        else:
